...
moco_overdose_calls <- read_csv("data/montgomery_2022_overdoses.csv")|> clean_names()

calls_by_zip <- moco_overdose_calls |> mutate(zip = as.character(zip))|>
...
moco_pop_overdose<- calls_by_zip |>
  # join md10 to md21 using the shared GEOID and NAME columns
  inner_join(md_zcta_pop, by=c('zip' = 'GEOID'))


 moco_pop_overdose <- moco_pop_overdose |> mutate(rate_per_1k = (total_calls/estimate)*1000)|> arrange(desc(rate_per_1k))
        
```

**Q4** Using [CensusReporter.org](https://censusreporter.org/), examine the zip codes with the highest and lowest rates and compare and contrast their demographic profiles. Is there a story here? I think there is a story there because the neighborhood with more overdose calls is in an interesting neighborhood - it has many parks and public school buildings. Based on Census Data, it also has a higher percentage of people of color, particularly Hispanic residents and tends to have households with lower incomes on average and more persons within the homes.
