...
pg_911_calls <- pg_911_calls |>
    mutate(date = date(datetime), month = month(date, label=TRUE), week = week(datetime))
...
pct_calls_week |>
  ggplot() + 
  geom_line(aes(x=week, y=pct_total)) +
  labs(
    title = 'Overdose calls in Prince George\'s County peak mid-year',
...
pg_zip_overdose <- calls_by_zip |>
  inner_join(md_zcta_pop, by=c('zipcode' = 'GEOID'))



pg_zip_overdose |>
  ggplot() + 
  geom_point(aes(x=estimate, y=total_calls, label = rownames(zipcode)))+
  geom_text() +
  labs(title="Zipcode overdose calls shown by 2019 Median Household Incomes", 
       y="Total Calls", 
       x="Median Household Income", 
       caption = "Source: Prince George\'s County EMS and United States Census Bureau") 
```
