...
clean_md_expenses <- maryland_expenses |>
...
write_csv(clean_md_expenses, "data/clean_md_expenses.csv")
...
#cleaned_md_expenses <- read_csv("data/clean-md-expenses-csv.csv")

# calculate top Facebook spenders

cleaned_md_expenses |>
mutate(
    clean_payee_name = str_to_upper(clean_payee_name)
)|>
  mutate(
    committee_name = str_to_upper(committee_name)
)|>
   group_by(committee_name, clean_payee_name, amount) |>
  summarise(
    count=n()
  ) |>
  filter(str_detect(clean_payee_name, '^FA|^META')) |>
  arrange(desc(amount))
```
...
**A2. CASA In Action is an example of a major spender that bought ads to advocate for political participation. Otherwise, many political committees for candidates were on a local scale rather than a national position scale.**
...
**A3. Wes Moore is not on the A1 list but he did run ads on Facebook, according to the ad library. Wes Moore gave the most money to SKDK.**

```{r}
cleaned_md_expenses|>
  filter(committee_name == "Moore  Wes For Maryland" | committee_name== "Moore Miller For Maryland Slate")|>
  group_by(payee_name, amount)|>
summarise(
    count=n()
  ) |>
  arrange(desc(amount))
...
**Q4.** The media expense data you cleaned also has a standardized purpose column. Write code to generate a list of the committees with the total amount of spending on online advertising. How does this list compare to the one from Q1 about top Facebook spenders? Of the top 5 committees who spent the most on Facebook ads from Q1, are there any that appear to have spent *all* of their online advertising dollars on Facebook? These are completely different committees that are paying for advertising but I think there are many more democrats and advocacy organizations that support candidates campaigns.

**A4.**

```{r}
cleaned_md_expenses |>
 group_by(committee_name, amount, expense_purpose) |>
  summarise(
   count=n()
  ) |>
...
**A5** candidates for local elections tend to spend more on advertisements, particularly on Facebook/Meta. There are also many nonprofit and election advocacy organizations that spend on Facebook/Meta so that they can encourage people to register or go out to vote. However, we don't know much about what larger candidates are spending their money on and how they are advocating for their own platform. I am curious to know what kinds of advertisements they find most beneficial to garner support or how they poll general interest, seeing as most Facebook ads that I saw were flyers for candidates and major candidates advertise in many more ways than this.
