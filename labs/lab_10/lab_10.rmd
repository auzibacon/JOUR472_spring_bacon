...
md_statewide_county <- md_statewide_county |> 
  mutate(total_gov = cox + moore + lashar + wallace + harding + sefcik + gov_writein, total_ag = brown + peroutka + ag_writein) |>
 mutate(pct_moore = moore/total_gov*100)|>
   mutate(pct_brown = brown/total_ag*100)|>
   mutate(pct_difference = pct_moore-pct_brown)
```
...
#Create the map
ggplot() +
  geom_sf(data=md_geom_votes, aes(fill=pct_difference)) 
```
...
ggplot() +
  geom_sf(data=md_geom_outages, aes(fill=outage_rate)) 
```
...
#Map out the change
ggplot() +
  geom_sf(data=md_geom_ev, aes(fill=pct_change)) 
```
...
md_population_ev <- md_geom_ev |> left_join(md_county_population, join_by(geoid))

md_population_ev <- md_population_ev |>
  mutate(per_capita = (march_2023/estimate)*100)
```
