...
combined_911 <- bind_rows(list(allegany_calls, carroll_calls, cecil_calls))
```
...
baltimore_911_with_demographics <- baltimore_calls |> left_join(maryland_zcta, join_by(zip_code == zcta5n))
```
...
Which zip code has the highest rate of calls per 1,000 people? 21217

Find the neighborhoods in that zip code that are listed in the data - you can use group_by or distinct to do this. What are some of those neighborhoods?
...
filter(pct_under_18 >= 75.0)|>
  ...
  mutate(rate = (total_calls/population) *1000)|>
...
At what rate do neighorhoods in Baltimore with a percentage of homeowners less than 25% receive calls?

A4:

```{r}
baltimore_911_with_demographics|>
filter(pct_homeowner < 25.0)|>
  group_by(zip_code, population)|>
