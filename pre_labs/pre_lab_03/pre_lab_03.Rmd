...
**Task** Run the following codeblock to count the number of payments. Looking at the result, what jumps out at you as potentially problematic? **Answer: The categories in this table are too varied, including an "L" and no values in different years.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does the function change the headers? Answer below. **It made column names lowercase/standardized.**
...
**Task** Run the following codeblock to use distinct() to get rid of duplicate rows. How many rows does the new dataframe have? Answer below. **17740**
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Does that zip code make sense to you? 21202 is at the highest with 20 grants. This makes sense because Baltimore is a top tourist destination in the state of Maryland, so I would expect it to have the most grants compared to other cities/counties.

```{r}
cleaned_md_grants_loans|>
   filter(str_detect(source, "MARYLAND TOURISM")) |> 
group_by(`zip5`) |>
  summarise(
    count=n()
  )|>
