...
```{r echo=FALSE}
census_api_key("629ea099665938460e9566b8aea75152bce66c2d", install=TRUE)
```
...
**Task** Run the following codeblock to calculate the percentage change and put the state with the largest percentage growth below. **Nevada had the largest percentage change with 35%.**
...
**Task** Run the following codeblock to find the Maryland county with the highest median household income and put the answer below. **Howard County has the highest median household income of \$129,549**
...
**Task** Run the following codeblock to create a chart showing the margin of error for each county. The current headline isn't very descriptive of the data. What should it be? **Average household income by Maryland county**
...
Is the difference between Calvert and Montgomery significant? **No, Calvert tends to have a higher median income with the MOE but otherwise they are about the same.**

Is the difference between Howard and everyone else significant? **It is significant when not compared to Calvert and Montgomery, as it is on the higher end with a lower range between MOE, so it is much different than other counties.**
