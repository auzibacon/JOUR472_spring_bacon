...
**Task** Run the following code to add a month column to our dataframe using lubridate and count the number of calls for each month. In the space below, explain what each step of the code below is doing in English. Note the "label=TRUE" part in the mutate statement. **Answer The first line of code creates a month column that extracts the "month" from the current date column. Then, it creates groups of the data by the different months and summarizes a count of the total calls (each count of data found for each month) and summarizes it into an additional column. The last line of code arranges it from most calls to least calls.**
...
**Task** Run the following code. Explain what you see after running the code in the space below. **This creates a bar graph which tallies the number of calls for each month and organizes the bars/months in order from least calls to most (left to right).**
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock? **This flips the X and Y axis of the bar graph so that the data presents horizontally instead of vertically.**
...
**Task** Run the following code. What do you think this does? **This saves the plot as an image file that can be called back to later.**
