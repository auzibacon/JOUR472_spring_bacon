...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer: chr (character) and dbl (double)**
...
**Answer: This tells you how many observations in the data set are a particular term. So, the term 201712, had 487 classes in the data set.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: This arranged the terms in descending order (largest to smallest count).**
...
**Answer: This also split the count by department after sorting by term, so the count specifically shows how many classes were offered by a specific department in a particular term rather than grouping all classes together.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? **Answer: It no longer considers department in its sorting but does include the total number of seats offered in each term.**
...
Compared to the previous codeblock, what has changed and how did that affect the result? What do the top terms have in common? **Answer: Both codeblocks are organized in descending order's number of seats, so the top terms have that in common. However, there are additional columns that the table is sorted by now, including the mean/median of seats per term.**
...
**Answer: This specifically arranges the data by number of classes and department, so the department with the most classes is organized at the top. It also adds minimum and maximum seats but this does not affect how it is organized or arranged.**
