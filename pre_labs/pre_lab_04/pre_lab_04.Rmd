...
**Task** Run the following code to group and count records in the Grantee column, and filter to examine only recipients that start with Y. Name a problem that you see that is preventing proper grouping. **Some grantees are repeated by have slightly different names or the formatting of the names are different, so they won't group.**
...
Run the following code to use the n_gram_merge() function to clean the data, then group by grantee_clean and Grantee to examine recipients that start with Y. Look at the differences. Does the changes it made vary substantially from the previous use of key_collision_merge? **No**
